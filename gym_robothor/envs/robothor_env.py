...
class RoboThorEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.observation_space = dict(rgb = spaces.Box(low=0, high=1, shape=(state['rgb'].shape[0], state['rgb'].shape[1], state['rgb'].shape[2])),
                                      compass = spaces.Box(low=0, high=1, shape=(state['compass'].shape[0],))
                                      )
        return state
...
def env_generator(split:str, device=None):
